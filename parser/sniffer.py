...
def packet_parser(mini_window_duration=1, max_mws=2, mode=0, verbose=0, file_name="/app_aware/dataset/dataset.csv", upload_time=600, upload_size=100):
    """
            ...
            if mw_time_end - mw_time_start > 1.5:
                ...
                for i in range(num_mw_inside):
                    ...
                    if mode == 0:
                        ...
                        end_timer = time.time()
                        if (end_timer - start_timer >= upload_time):
                            start_timer = time.time()
                            size = get_file_size_mb_(file_name)
                            if size != 0:
                                if size <= upload_size:
                                    upload_data(file_name)
                                else:
                                    keep_latest_data(file_name)            
...
def upload_data(file_path='/app_aware/dataset/dataset.csv'):
    git_dir = '/app_aware'
    g = git.cmd.Git(git_dir)
    g.pull()
    repo_dir = '/app_aware'
    repo = Repo(repo_dir)
    commit_message = 'Commit from the pipeline'
    repo.index.add(file)
    repo.index.commit(commit_message)
    origin = repo.remote('origin')
    origin.push()

def get_file_size_mb_(file_path='/app_aware/dataset/dataset.csv'):
    size = os.path.getsize(file_path)
    size = round(size / (1024 * 1024), 3)
    return size  

def keep_latest_data(file_path='/app_aware/dataset/dataset.csv'):
    pass
